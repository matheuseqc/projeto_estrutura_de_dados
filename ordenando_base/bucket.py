...
def bucket_sort(file_path):
    ...
    num_buckets = len(set(row[0] for row in rows)) # número de buckets é igual ao número de valores distintos na coluna de índice 0
    ...
    with open('ordenando_base/bucketSort.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(header)
        writer.writerows(sorted_rows)

bucket_sort('ordenando_base/shuffled_data.csv')
